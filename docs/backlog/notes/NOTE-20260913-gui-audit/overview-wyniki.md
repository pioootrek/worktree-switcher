...
## Korekta wysokości klikanych kart

Po zgłoszeniu właściciela poprawiono rozciąganie CardContent i Button na całą wysokość karty metryki. Treść pozostaje wyrównana do góry. W istniejącej karcie, viewport 1195 px: przed zmianą Worktrees miał kartę 138 px i przycisk 110 px; po HMR oba mają 138 px. Wszystkie cztery etykiety mają identyczny odstęp 17 px od góry, przyciski wypełniają karty, brak przepełnienia strony. Drobna korekta klas CSS: git diff --check i ponowna weryfikacja DOM, bez ponawiania builda i testów. Przyrostowy patch zaktualizowano, reverse check przeszedł. Aktywacja backendu nadal oczekuje zgody.
