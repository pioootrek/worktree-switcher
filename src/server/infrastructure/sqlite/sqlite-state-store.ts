...
export class SqliteStateStore implements StateStore, IdentityStore, KnowledgeStore, RemoteVerificationStore, RemoteVerificationAttemptStore, RemoteVerificationProvisioningStore {
  ...
  exportKnowledgeProject(projectId: string): KnowledgeProjectSnapshot | null {
    ...
    const history = table("knowledge_history");
  ...
  importKnowledgeProject(snapshot: KnowledgeProjectSnapshot): void {
    this.database.transaction(() => {
      ...
      if (typeof projectId !== "string" || this.database.prepare("SELECT 1 FROM knowledge_projects WHERE id = ?").get(projectId)) throw new Error("Knowledge project already exists or has an invalid ID.");
      for (const principalId of snapshot.requiredPrincipals) if (!this.database.prepare("SELECT 1 FROM remote_principals WHERE id = ?").get(principalId)) throw new Error(`Required historical principal is missing: ${principalId}`);
      const columns:Record<string,string[]>={
        ...
        knowledge_history:["id","project_id","record_kind","record_id","operation","previous_json","principal_id","authentication_method","revision","created_at"],
      ...
      const insert = (table: string, rows: Array<Record<string, unknown>>) => {
        ...
        for (const row of rows) {
          if(Object.keys(row).sort().join("\0")!==[...expected].sort().join("\0")) throw new Error(`Invalid ${table} record shape.`);
          if(table!=="knowledge_projects"&&row.project_id!==projectId) throw new Error(`Foreign project record in ${table}.`);
          this.database.prepare(`INSERT INTO ${table} (${expected.join(",")}) VALUES (${expected.map(() => "?").join(",")})`).run(...expected.map(key => row[key]));
        }
      };
