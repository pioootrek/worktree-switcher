...
export type ServerMode = "normal" | "gate" | "early-exit" | "timeout";
...
async function createRepository(base: string, name: string): Promise<{ main: string; alternate: string }> {
  ...
  await writeFile(join(main, "server.mjs"), [
    'import { existsSync, readFileSync } from "node:fs";', 'import { createServer } from "node:http";', 'import { randomUUID } from "node:crypto";',
...
export async function startControllerFixture(projectCount = 3): Promise<ControllerFixture> {
  ...
  const stop = async () => { if (child) await closeChild(child); child = undefined; };
...
export async function endpointUnavailable(project: FixtureProject, timeoutMs = 10_000): Promise<void> {
  await waitFor(async () => { try { await fetch(`http://127.0.0.1:${project.port}`, { signal: AbortSignal.timeout(300) }); return null; } catch { return true; } }, timeoutMs, () => `Project ${project.name} listener remained reachable.`);
}
async function closeChild(child: ChildProcess): Promise<void> { if (child.exitCode !== null) return; child.kill("SIGTERM"); await waitFor(() => child.exitCode !== null ? true : null, WAIT_MS, () => "Fixture controller did not shut down cleanly."); if (child.exitCode !== 0) throw new Error(`Fixture controller exited uncleanly (${child.exitCode}).`); }
