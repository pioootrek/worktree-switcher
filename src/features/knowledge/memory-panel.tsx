  ...
  return <div className="space-y-5">
    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(200px,1fr)_minmax(0,2fr)]">
      ...
      {record && <article className="min-w-0 space-y-4 rounded-xl border border-border p-4">
        ...
        <h4 className="font-medium">{t("knowledge.sources")}</h4><ul className="space-y-2">{record.sources.map((source, index) => <li className="break-all text-sm" key={index}>{source.kind === "reply" ? `${source.id} · r${source.revision}` : <a className="underline" href={sourceHref(projectId, source)}>{source.kind === "external" ? source.label : `${source.id} · r${source.revision}`}</a>}</li>)}</ul>
