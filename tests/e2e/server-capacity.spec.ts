...
test("the real dashboard enforces, reuses, and lowers server capacity", async ({ page }) => {
  ...
  try {
    ...
    await (await selectDashboardProject(page, "project-a")).getByRole("button", { name: "Start", exact: true }).click();
    await (await selectDashboardProject(page, "project-b")).getByRole("button", { name: "Start", exact: true }).click();
    ...
    await (await selectDashboardProject(page, "project-c")).getByRole("button", { name: "Start", exact: true }).click();
    ...
    await (await selectDashboardProject(page, "project-a")).getByRole("button", { name: "Stop", exact: true }).click();
    ...
    await expect((await selectDashboardProject(page, "project-b")).getByRole("button", { name: "Stop", exact: true })).toBeVisible();
    await expect((await selectDashboardProject(page, "project-c")).getByRole("button", { name: "Stop", exact: true })).toBeVisible();
